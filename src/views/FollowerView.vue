		...
		<table class="table table-image">
            ...
            <tr v-for="(item) in friends" :key="item.id">
             ...
             <td class="w-25" v-if="item.imageUrl==null">
              ...
              <td>{{ item.name }}</td>
              <td><button class="btn btn-primary" @click="unfollow(item.id)">Remove</button></td>
...
<script>
    ...
    export default {
        ...
        methods: {
           async getFriends() {
               var friends = await axios.get("/follower/search");
                this.friends=friends.data;
                console.log(friends.data);
            },
            async unfollow(id){
                var response = await axios.get("/remove/follower/?id="+id);
